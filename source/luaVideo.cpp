...
int argc = lua_gettop(L);
	...
	if (src->samplerate != 0 && src->audio_size != 0){
		...
		if (src->audiotype == 1){
			...
			My_CSND_playsound(ch1, CSND_LOOPMODE_NORMAL, CSND_ENCODING_PCM16, src->samplerate, (u32*)src->audiobuf, (u32*)src->audiobuf, src->mem_size, 0xFFFF, 0xFFFF);
		}else{
			if (src->audiocodec == 0){
			...
			My_CSND_playsound(src->ch1, CSND_LOOPMODE_NORMAL, CSND_ENCODING_PCM16, src->samplerate, (u32*)src->audiobuf, (u32*)src->audiobuf, src->mem_size/2, 0xFFFF, 0);
			My_CSND_playsound(src->ch2, CSND_LOOPMODE_NORMAL, CSND_ENCODING_PCM16, src->samplerate, (u32*)src->audiobuf2, (u32*)src->audiobuf2, src->mem_size/2, 0, 0xFFFF);
		}
